...
func TestServer_OpenAPI_AndRootRedirect(t *testing.T) {
    ...
    if resp.StatusCode != http.StatusFound { t.Fatalf("root status=%d", resp.StatusCode) }

    // openapi.json should return 200
    r2, err := http.Get(ts.URL + "/api/openapi.json")
    if err != nil { t.Fatalf("openapi.json: %v", err) }
    if r2.StatusCode != http.StatusOK { t.Fatalf("openapi.json status=%d", r2.StatusCode) }
}
