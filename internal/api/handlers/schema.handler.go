...
func (h *SchemaHandler) UpsertMetric(c *gin.Context) {
    ...
    m := repo.MetricDef{TenantID: req.TenantID, Metric: req.Metric, Description: req.Description, Owner: req.Owner, Tags: req.Tags, UpdatedAt: time.Now()}
    if err := h.repo.UpsertMetric(c.Request.Context(), m, req.Author); err != nil {
        h.logger.Error("metric upsert failed", "error", err)
        c.JSON(http.StatusInternalServerError, gin.H{"error": "upsert failed"}); return
    }
...
func (h *SchemaHandler) UpsertLogField(c *gin.Context) {
    ...
    if req.TenantID == "" { req.TenantID = c.GetString("tenant_id") }
    f := repo.LogFieldDef{TenantID: req.TenantID, Field: req.Field, Type: req.Type, Description: req.Description, Tags: req.Tags, Examples: req.Examples, UpdatedAt: time.Now()}
    if err := h.repo.UpsertLogField(c.Request.Context(), f, req.Author); err != nil {
...
func (h *SchemaHandler) ListTraceServiceVersions(c *gin.Context) {
    ...
    if err != nil { h.logger.Error("list trace service versions failed", "error", err); c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to list versions"}); return }
    c.JSON(http.StatusOK, gin.H{"versions": out})
}
...
func (h *SchemaHandler) ListTraceOperationVersions(c *gin.Context) {
    ...
    if err != nil { h.logger.Error("list trace operation versions failed", "error", err); c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to list versions"}); return }
    c.JSON(http.StatusOK, gin.H{"versions": out})
}
...
func (h *SchemaHandler) BulkUpsertTraceServicesCSV(c *gin.Context) {
    if limited := h.enforceQuota(c, "traces_services", 20); limited { return }
    c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, h.maxUploadBytes)
    if err := c.Request.ParseMultipartForm(6 << 20); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid multipart form or file too large"}); return }
    file, header, err := c.Request.FormFile("file"); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"file is required"}); return }
    defer file.Close()
    // MIME allowlist + sniff
    allowedCT := map[string]struct{}{"text/csv":{}, "application/vnd.ms-excel":{}, "text/plain":{}}
    if ct := header.Header.Get("Content-Type"); ct != "" { if _, ok := allowedCT[strings.ToLower(ct)]; !ok { /* sniff below */ } }
    var sniff [512]byte; n, _ := file.Read(sniff[:]); if det := http.DetectContentType(sniff[:n]); det != "application/octet-stream" { if _, ok := allowedCT[det]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error":"unsupported content type"}); return } }
    if _, err := file.Seek(0, io.SeekStart); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"failed to read file"}); return }
    r := csv.NewReader(file); r.TrimLeadingSpace = true
    headerRow, err := r.Read(); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"empty csv"}); return }
    if len(headerRow) > 32 { c.JSON(http.StatusBadRequest, gin.H{"error":"too many columns"}); return }
    for _, col := range headerRow { if !utf8.ValidString(col) { c.JSON(http.StatusBadRequest, gin.H{"error":"invalid utf-8 in header"}); return } }
    idx := map[string]int{}; for i, col := range headerRow { idx[strings.ToLower(strings.TrimSpace(col))] = i }
    ...
    if _, ok := idx["service"]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error":"missing column: service"}); return }
    tenantOverride := c.GetString("tenant_id")
    count := 0; var errs []string; const maxRows = 10000; rows := 0
    for {
        rec, err := r.Read(); if err == io.EOF { break }; if err != nil { errs = append(errs, "read error"); continue }
        rows++; if rows > maxRows { errs = append(errs, "row limit exceeded"); break }
        for i := range rec { if !utf8.ValidString(rec[i]) { rec[i] = sanitizeCSVCell(rec[i]) }; rec[i] = sanitizeCSVCell(rec[i]) }
        ...
        service := get("service"); if service == "" { errs = append(errs, "missing service"); continue }
        purpose := get("purpose"); owner := get("owner"); tags := get("tags_json"); author := get("author")
        var tagsObj map[string]any; if tags != "" { _ = json.Unmarshal([]byte(tags), &tagsObj) }
        if err := h.repo.UpsertTraceServiceWithAuthor(c.Request.Context(), tenant, service, purpose, owner, tagsObj, author); err != nil { errs = append(errs, "service upsert failed: "+service) } else { count++ }
    }
...
func (h *SchemaHandler) BulkUpsertTraceOperationsCSV(c *gin.Context) {
    if limited := h.enforceQuota(c, "traces_operations", 20); limited { return }
    c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, h.maxUploadBytes)
    if err := c.Request.ParseMultipartForm(6 << 20); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"invalid multipart form or file too large"}); return }
    file, header, err := c.Request.FormFile("file"); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"file is required"}); return }
    defer file.Close()
    ...
    for k := range idx { if _, ok := allowed[k]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error":"unknown column: "+k}); return } }
    if _, ok := idx["service"]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error":"missing column: service"}); return }
    if _, ok := idx["operation"]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error":"missing column: operation"}); return }
    tenantOverride := c.GetString("tenant_id")
    count := 0; var errs []string; const maxRows = 10000; rows := 0
    for {
        rec, err := r.Read(); if err == io.EOF { break }; if err != nil { errs = append(errs, "read error"); continue }
        ...
        get := func(k string) string { if j, ok := idx[k]; ok && j < len(rec) { return strings.TrimSpace(rec[j]) } ; return "" }
        tenant := get("tenant_id"); if tenant == "" { tenant = tenantOverride }
        service := get("service"); operation := get("operation"); if service == "" || operation == "" { errs = append(errs, "missing service/operation"); continue }
        // Ensure service exists to maintain sanity that operations are per service
        if _, err := h.repo.GetTraceService(c.Request.Context(), tenant, service); err != nil { errs = append(errs, "undefined service: "+service); continue }
        purpose := get("purpose"); owner := get("owner"); tags := get("tags_json"); author := get("author")
        var tagsObj map[string]any; if tags != "" { _ = json.Unmarshal([]byte(tags), &tagsObj) }
        if err := h.repo.UpsertTraceOperationWithAuthor(c.Request.Context(), tenant, service, operation, purpose, owner, tagsObj, author); err != nil { errs = append(errs, "operation upsert failed: "+service+":"+operation) } else { count++ }
    }
...
func (h *SchemaHandler) BulkUpsertMetricsCSV(c *gin.Context) {
    ...
    if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "file is required"}); return }
    defer file.Close()
    ...
    if _, ok := allowedCT[detected]; !ok && detected != "application/octet-stream" {
        c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported content type"}); return
    }
    if _, err := file.Seek(0, io.SeekStart); err != nil {
        c.JSON(http.StatusBadRequest, gin.H{"error": "failed to read file"}); return
    }
    ...
    if len(headerRow) > 64 { c.JSON(http.StatusBadRequest, gin.H{"error": "too many columns"}); return }
    for _, col := range headerRow {
        if !utf8.ValidString(col) { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid utf-8 in header"}); return }
    }
    ...
    for _, col := range required {
        if _, ok := idx[col]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error": "missing column: " + col}); return }
    }
    ...
    for {
        ...
        rows++
        if rows > maxRows { rowErrs = append(rowErrs, "row limit exceeded"); break }
        // Validate UTF-8 and sanitize to mitigate CSV injection
        for i := range rec {
            if !utf8.ValidString(rec[i]) { rec[i] = sanitizeCSVCell(rec[i]) }
            rec[i] = sanitizeCSVCell(rec[i])
        }
        get := func(name string) string { if j, ok := idx[name]; ok && j < len(rec) { return strings.TrimSpace(rec[j]) }; return "" }
        tenant := get("tenant_id")
        if tenant == "" { tenant = tenantOverride }
        metric := get("metric")
        if metric == "" { rowErrs = append(rowErrs, "missing metric"); continue }
        ...
        if label != "" {
            ...
            if lallowed != "" { _ = json.Unmarshal([]byte(lallowed), &allowed) }
    ...
    for _, mr := range metricsSeen {
        ...
        if mr.tags != "" { _ = json.Unmarshal([]byte(mr.tags), &tagsObj) }
...
func (h *SchemaHandler) BulkUpsertLogFieldsCSV(c *gin.Context) {
    if limited := h.enforceQuota(c, "logs", 20); limited { return }
    ...
    for k := range idx { if _, ok := allowed[k]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error": "unknown column: "+k}); return } }
    if _, ok := idx["field"]; !ok { c.JSON(http.StatusBadRequest, gin.H{"error": "missing column: field"}); return }
    ...
    for {
        ...
        get := func(name string) string { if j, ok := idx[name]; ok && j < len(rec) { return strings.TrimSpace(rec[j]) }; return "" }
        tenant := get("tenant_id"); if tenant == "" { tenant = tenantOverride }
        field := get("field"); if field == "" { rowErrs = append(rowErrs, "missing field"); continue }
        typ := get("type"); desc := get("description"); tags := get("tags_json"); examples := get("examples_json"); author := get("author")
        var tagsObj, exObj map[string]any
        if tags != "" { _ = json.Unmarshal([]byte(tags), &tagsObj) }
        if examples != "" { _ = json.Unmarshal([]byte(examples), &exObj) }
        f := repo.LogFieldDef{TenantID: tenant, Field: field, Type: typ, Description: desc, Tags: tagsObj, Examples: exObj, UpdatedAt: time.Now()}
        if err := h.repo.UpsertLogField(c.Request.Context(), f, author); err != nil { rowErrs = append(rowErrs, "field upsert failed: "+field) } else { fieldsCount++ }
    }
...
func (h *SchemaHandler) SampleCSVLogFields(c *gin.Context) {
    ...
    _ = w.Write(header)
    if h.logsService == nil { w.Flush(); return }
    tenantID := c.GetString("tenant_id")
    fields, err := h.logsService.GetFields(c.Request.Context(), tenantID)
    if err == nil {
        for _, f := range fields { _ = w.Write([]string{"", f, "", "", "{}", "{}", ""}) }
    }
    w.Flush()
}
...
func (h *SchemaHandler) enforceQuota(c *gin.Context, kind string, limit int) bool {
    if h.cache == nil || limit <= 0 { return false }
    ...
    // get existing
    var count int
    if b, err := h.cache.Get(c.Request.Context(), key); err == nil { _, _ = fmt.Sscanf(string(b), "%d", &count) }
    if count >= limit {
...
func (h *SchemaHandler) getLabelNamesForMetric(c *gin.Context, tenantID, metric string) ([]string, error) {
    if h.metricsService == nil { return nil, fmt.Errorf("no metrics service") }
...
func sanitizeCSVCell(s string) string {
    if !utf8.ValidString(s) { s = strings.ToValidUTF8(s, "") }
