...
func TestRBAC_Session_Config(t *testing.T) {
    ...
    if w.Code != http.StatusOK { t.Fatalf("rbac roles status=%d", w.Code) }

    // RBAC create invalid
    w = httptest.NewRecorder()
    r.ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/api/v1/rbac/roles", bytes.NewReader([]byte("{}"))))
    if w.Code != http.StatusBadRequest { t.Fatalf("rbac create invalid=%d", w.Code) }
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("rbac assign=%d body=%s", w.Code, w.Body.String()) }

    // Sessions active
    w = httptest.NewRecorder()
    r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/v1/sessions/active", nil))
    if w.Code != http.StatusOK { t.Fatalf("sessions active=%d", w.Code) }

    // Sessions user
    w = httptest.NewRecorder()
    r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/v1/sessions/user/u1", nil))
    if w.Code != http.StatusOK { t.Fatalf("sessions user=%d", w.Code) }
    ...
    if w.Code != http.StatusOK { t.Fatalf("invalidate with session should 200, got=%d", w.Code) }
    ...
    if w.Code != http.StatusBadRequest { t.Fatalf("invalidate missing should 400, got=%d", w.Code) }

    // Config get user settings (uses session)
    w = httptest.NewRecorder()
    r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/v1/config/user-settings", nil))
    if w.Code != http.StatusOK { t.Fatalf("get settings=%d", w.Code) }
    ...
    req.Header.Set("Content-Type", "application/json")
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("update settings=%d body=%s", w.Code, w.Body.String()) }

    // Config list datasources
    w = httptest.NewRecorder()
    r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/v1/config/datasources", nil))
    if w.Code != http.StatusOK { t.Fatalf("datasources=%d", w.Code) }
    ...
    req.Header.Set("Content-Type", "application/json")
    r.ServeHTTP(w, req)
    if w.Code != http.StatusCreated { t.Fatalf("add datasource=%d", w.Code) }

    // Config integrations
    w = httptest.NewRecorder()
    r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/v1/config/integrations", nil))
    if w.Code != http.StatusOK { t.Fatalf("integrations=%d", w.Code) }
}
