...
func TestExtractToken_Sources(t *testing.T) {
    ...
    if got := extractToken(c); got != "qt" { t.Fatalf("query token got %q", got) }

    c.Request = httptest.NewRequest(http.MethodGet, "/x", nil)
    c.Request.Header.Set("X-Session-Token", "xs")
    if got := extractToken(c); got != "xs" { t.Fatalf("x-session got %q", got) }

    c.Request = httptest.NewRequest(http.MethodGet, "/x", nil)
    c.Request.Header.Set("Authorization", "Bearer abcd")
    if got := extractToken(c); got != "abcd" { t.Fatalf("auth got %q", got) }
}
...
func TestValidateJWTToken_OK(t *testing.T) {
    ...
    if sess.UserID != "u1" || sess.TenantID != "t1" { t.Fatalf("unexpected session: %+v", sess) }
    if len(sess.Roles) == 0 { t.Fatalf("expected roles") }
}
