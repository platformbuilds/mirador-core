...
func BuildFlameGraphFromJaegerWithMode(traceID string, spans []map[string]any, processes map[string]any, mode string) FlameNode {
    ...
    toInt64 := func(v any) int64 {
        ...
        case string:
            ...
            if strings.Contains(x, "T") && (strings.Contains(x, "-") || strings.Contains(x, ":")) {
                if t, err := time.Parse(time.RFC3339Nano, x); err == nil { return t.UnixMicro() }
                if t, err := time.Parse(time.RFC3339, x); err == nil { return t.UnixMicro() }
            }
            // Parse numeric string; ignore non-digits suffix/prefix gracefully (e.g., "123000" or "123ms")
            var digits []rune
            for _, r := range x { if r >= '0' && r <= '9' { digits = append(digits, r) } }
            if len(digits) == 0 { return 0 }
            var n int64 = 0
            for _, r := range digits { n = n*10 + int64(r-'0') }
            return n
        default:
            return 0
        }
    ...
    getStartMicros := func(span map[string]any) int64 {
        ...
        if v, ok := span["startTime"]; ok { return toInt64(v) }
        if v, ok := span["startTimeMicros"]; ok { return toInt64(v) }
        if v, ok := span["start_time"]; ok { return toInt64(v) }
        // Unix nanos/millis variants
        if v, ok := span["startTimeUnixNano"]; ok { return toInt64(v) / 1000 }
        if v, ok := span["startTimeUnixMillis"]; ok { return toInt64(v) * 1000 }
        return 0
    }
    getDurationMicros := func(span map[string]any) int64 {
        if v, ok := span["duration"]; ok { return toInt64(v) }
        if v, ok := span["durationMicros"]; ok { return toInt64(v) }
        if v, ok := span["duration_us"]; ok { return toInt64(v) }
        if v, ok := span["durationMs"]; ok { return toInt64(v) * 1000 }
        if v, ok := span["duration_ms"]; ok { return toInt64(v) * 1000 }
        if v, ok := span["durationNanos"]; ok { return toInt64(v) / 1000 }
        if v, ok := span["duration_ns"]; ok { return toInt64(v) / 1000 }
        return 0
    }
    ...
    for _, s := range spans {
        ...
        if startMs > 0 {
            if minStart == 0 || startMs < minStart { minStart = startMs }
            if end := startMs + ms; end > maxEnd { maxEnd = end }
        }
    }
    for _, w := range byID {
    ...
    build = func(w *spanWrap) FlameNode {
        ...
        if proc, ok := processes[w.process].(map[string]interface{}); ok {
            if srv, ok := proc["serviceName"].(string); ok { node.Service = srv }
        }
        // children (ordered by start time)
        var kids []*spanWrap
        for _, c := range byID {
            if c.parentID == w.id { kids = append(kids, c) }
        }
        if len(kids) > 1 {
        ...
        for _, c := range kids { node.Children = append(node.Children, build(c)) }
        if mode == string(FlameSelf) {
            var childSum int64
            for _, ch := range node.Children { childSum += ch.Value }
    ...
    if len(roots) == 1 {
        ...
        if minStart > 0 && maxEnd > minStart { root.Value = maxEnd - minStart }
        // include total duration in label
        root.Name = root.Name + " (" + itoa(root.Value) + " ms)"
        return root
    }
    root := FlameNode{Name: "trace " + traceID, Value: 0}
    for _, r := range roots { root.Children = append(root.Children, build(r)) }
    if minStart > 0 && maxEnd > minStart { root.Value = maxEnd - minStart }
    root.Name = root.Name + " (" + itoa(root.Value) + " ms)"
    return root
}
...
func MergeFlameTrees(a *FlameNode, b FlameNode) {
    ...
    for i := range a.Children { idx[a.Children[i].Name] = &a.Children[i] }
