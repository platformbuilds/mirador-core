...
func (r *SchemaRepo) UpsertMetric(ctx context.Context, m MetricDef, author string) error {
    ...
    if _, err := tx.ExecContext(ctx,
        ...
        m.TenantID, m.Metric, m.Description, m.Owner, string(tagsJSON)); err != nil { return err }
    // bump version counter
    if _, err := tx.ExecContext(ctx, `UPDATE metric_def SET current_version = current_version + 1 WHERE tenant_id=? AND metric=?`, m.TenantID, m.Metric); err != nil { return err }
    // insert version row from current
    var ver int64
    if err := tx.QueryRowContext(ctx, `SELECT current_version FROM metric_def WHERE tenant_id=? AND metric=?`, m.TenantID, m.Metric).Scan(&ver); err != nil { return err }
    payload, _ := json.Marshal(m)
    if _, err := tx.ExecContext(ctx, `INSERT INTO metric_def_versions(tenant_id,metric,version,payload,author) VALUES (?,?,?,?,?)`, m.TenantID, m.Metric, ver, string(payload), author); err != nil { return err }
    return tx.Commit()
}
...
func (r *SchemaRepo) GetMetric(ctx context.Context, tenantID, metric string) (*MetricDef, error) {
    ...
    if err := row.Scan(&desc, &owner, &tagsRaw, &updated); err != nil { return nil, err }
    var tags map[string]any
    if tagsRaw.Valid { _ = json.Unmarshal([]byte(tagsRaw.String), &tags) }
...
func (r *SchemaRepo) UpsertLogField(ctx context.Context, f LogFieldDef, author string) error {
    ...
    if _, err := tx.ExecContext(ctx,
        ...
        f.TenantID, f.Field, f.Type, f.Description, string(tagsJSON), string(exJSON)); err != nil { return err }
    // bump & version
    var ver int64
    // implement version table by selecting MAX(version)+1
    if err := tx.QueryRowContext(ctx, `SELECT IFNULL(MAX(version),0)+1 FROM log_field_def_versions WHERE tenant_id=? AND field=?`, f.TenantID, f.Field).Scan(&ver); err != nil { return err }
    payload, _ := json.Marshal(f)
    if _, err := tx.ExecContext(ctx, `INSERT INTO log_field_def_versions(tenant_id,field,version,payload,author) VALUES (?,?,?,?,?)`, f.TenantID, f.Field, ver, string(payload), author); err != nil { return err }
    return tx.Commit()
}
...
func (r *SchemaRepo) GetLogField(ctx context.Context, tenantID, field string) (*LogFieldDef, error) {
    ...
    var updated time.Time
    if err := row.Scan(&typ, &desc, &tagsRaw, &exRaw, &updated); err != nil { return nil, err }
    var tags, ex map[string]any
    if tagsRaw.Valid { _ = json.Unmarshal([]byte(tagsRaw.String), &tags) }
    if exRaw.Valid { _ = json.Unmarshal([]byte(exRaw.String), &ex) }
...
// Deprecated convenience wrappers removed: use UpsertTraceServiceWithAuthor / UpsertTraceOperationWithAuthor

// Versioned upserts with author for traces service/operation
func (r *SchemaRepo) UpsertTraceServiceWithAuthor(ctx context.Context, tenantID, service, purpose, owner string, tags map[string]any, author string) error {
    ...
    if _, err := tx.ExecContext(ctx,
        ...
        tenantID, service, purpose, owner, string(tagsJSON)); err != nil { return err }
    var ver int64
    if err := tx.QueryRowContext(ctx, `SELECT IFNULL(MAX(version),0)+1 FROM traces_service_def_versions WHERE tenant_id=? AND service=?`, tenantID, service).Scan(&ver); err != nil { return err }
    ...
    if _, err := tx.ExecContext(ctx, `INSERT INTO traces_service_def_versions(tenant_id,service,version,payload,author) VALUES (?,?,?,?,?)`, tenantID, service, ver, string(payload), author); err != nil { return err }
    return tx.Commit()
}

func (r *SchemaRepo) UpsertTraceOperationWithAuthor(ctx context.Context, tenantID, service, operation, purpose, owner string, tags map[string]any, author string) error {
    tagsJSON, _ := json.Marshal(tags)
    tx, err := r.DB.BeginTx(ctx, nil)
    if err != nil { return err }
    defer func(){ _ = tx.Rollback() }()
    if _, err := tx.ExecContext(ctx,
        `INSERT INTO traces_operation_def (tenant_id, service, operation, purpose, owner, tags)
         VALUES (?,?,?,?,?,?)
         ON DUPLICATE KEY UPDATE purpose=VALUES(purpose), owner=VALUES(owner), tags=VALUES(tags), updated_at=CURRENT_TIMESTAMP`,
        tenantID, service, operation, purpose, owner, string(tagsJSON)); err != nil { return err }
    var ver int64
    if err := tx.QueryRowContext(ctx, `SELECT IFNULL(MAX(version),0)+1 FROM traces_operation_def_versions WHERE tenant_id=? AND service=? AND operation=?`, tenantID, service, operation).Scan(&ver); err != nil { return err }
    payload, _ := json.Marshal(map[string]any{
        "tenantId": tenantID,
        "service": service,
        "operation": operation,
    ...
    if _, err := tx.ExecContext(ctx, `INSERT INTO traces_operation_def_versions(tenant_id,service,operation,version,payload,author) VALUES (?,?,?,?,?,?)`, tenantID, service, operation, ver, string(payload), author); err != nil { return err }
    return tx.Commit()
}
...
func (r *SchemaRepo) GetTraceService(ctx context.Context, tenantID, service string) (*TraceServiceDef, error) {
    ...
    var tagsRaw sql.NullString
    var updated time.Time
    if err := row.Scan(&purpose, &owner, &tagsRaw, &updated); err != nil { return nil, err }
    var tags map[string]any
    if tagsRaw.Valid { _ = json.Unmarshal([]byte(tagsRaw.String), &tags) }
...
func (r *SchemaRepo) ListTraceServiceVersions(ctx context.Context, tenantID, service string) ([]VersionInfo, error) {
    ...
    for rows.Next() {
        var v VersionInfo
        if err := rows.Scan(&v.Version, &v.Author, &v.CreatedAt); err != nil { return nil, err }
        out = append(out, v)
    }
    return out, nil
}
...
func (r *SchemaRepo) GetTraceServiceVersion(ctx context.Context, tenantID, service string, version int64) (map[string]any, VersionInfo, error) {
    ...
    var vi VersionInfo
    if err := row.Scan(&payloadStr, &vi.Author, &vi.CreatedAt); err != nil { return nil, VersionInfo{}, err }
    vi.Version = version
    var payload map[string]any
    if payloadStr.Valid { _ = json.Unmarshal([]byte(payloadStr.String), &payload) }
...
func (r *SchemaRepo) GetMetricLabelDefs(ctx context.Context, tenantID, metric string, labels []string) (map[string]*MetricLabelDef, error) {
    ...
    for _, l := range labels { placeholders = append(placeholders, "?"); args = append(args, l) }
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&label, &typ, &req, &allowed, &desc); err != nil { return nil, err }
        var allowedMap map[string]any
        if allowed.Valid { _ = json.Unmarshal([]byte(allowed.String), &allowedMap) }
