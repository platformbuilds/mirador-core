...
func (s *VictoriaTracesService) GetServices(ctx context.Context, tenantID string) ([]string, error) {
    if len(s.children) > 0 {
        ...
        for _, svc := range services {
            go func(svc *VictoriaTracesService) { o, e := svc.GetServices(ctx, tenantID); ch <- struct{ out []string; err error }{o, e} }(svc)
        }
        ...
        for i := 0; i < len(services); i++ {
            ...
            for _, v := range r.out { set[v] = struct{}{} }
            okCount++
        }
        if okCount == 0 { return nil, fmt.Errorf("all traces sources failed") }
...
func (s *VictoriaTracesService) GetTrace(ctx context.Context, traceID, tenantID string) (*models.Trace, error) {
    if len(s.children) > 0 {
        ...
        if func() bool { s.mu.Lock(); defer s.mu.Unlock(); return len(s.endpoints) > 0 }() { services = append(services, s) }
        services = append(services, s.children...)
        ch := make(chan struct{ tr *models.Trace; err error }, len(services))
        for _, svc := range services {
            go func(svc *VictoriaTracesService) { t, e := svc.GetTrace(ctx, traceID, tenantID); ch <- struct{ tr *models.Trace; err error }{t, e} }(svc)
        }
        var firstErr error
        for i := 0; i < len(services); i++ {
            r := <-ch
            if r.err == nil && r.tr != nil { return r.tr, nil }
            if firstErr == nil { firstErr = r.err }
        }
        if firstErr == nil { firstErr = fmt.Errorf("trace not found") }
        return nil, firstErr
    }
    endpoint := s.selectEndpoint()
...
func (s *VictoriaTracesService) SearchTraces(ctx context.Context, request *models.TraceSearchRequest) (*models.TraceSearchResult, error) {
    if len(s.children) > 0 {
        ...
        if func() bool { s.mu.Lock(); defer s.mu.Unlock(); return len(s.endpoints) > 0 }() { services = append(services, s) }
        services = append(services, s.children...)
        ch := make(chan struct{ res *models.TraceSearchResult; err error }, len(services))
        for _, svc := range services {
            go func(svc *VictoriaTracesService) { r, e := svc.SearchTraces(ctx, request); ch <- struct{ res *models.TraceSearchResult; err error }{r, e} }(svc)
        }
        ...
        for i := 0; i < len(services); i++ {
            r := <-ch
            if r.err != nil || r.res == nil { continue }
            if len(r.res.Traces) > 0 { merged = append(merged, r.res.Traces...) }
            total += r.res.Total
            okCount++
        }
        if okCount == 0 { return nil, fmt.Errorf("all traces sources failed") }
    ...
    if len(dbConfig.MetricsSources) > 0 && s.Metrics != nil && s.Metrics.children != nil {
        for i, mc := range dbConfig.MetricsSources {
            if mc.Discovery.Enabled && i < len(s.Metrics.children) {
                ...
                if len(mc.Endpoints) > 0 { child.ReplaceEndpoints(mc.Endpoints) }
                cfg := mc.Discovery
                discovery.StartDNSDiscovery(ctx, discovery.DNSConfig{
                    Enabled:        true,
                    Service:        cfg.Service,
    ...
    if len(dbConfig.LogsSources) > 0 && s.Logs != nil && s.Logs.children != nil {
        for i, lc := range dbConfig.LogsSources {
            if lc.Discovery.Enabled && i < len(s.Logs.children) {
                ...
                if len(lc.Endpoints) > 0 { child.ReplaceEndpoints(lc.Endpoints) }
                cfg := lc.Discovery
                discovery.StartDNSDiscovery(ctx, discovery.DNSConfig{
                    Enabled:        true,
                    Service:        cfg.Service,
    ...
    if len(dbConfig.TracesSources) > 0 && s.Traces != nil && s.Traces.children != nil {
        for i, tc := range dbConfig.TracesSources {
            if tc.Discovery.Enabled && i < len(s.Traces.children) {
                ...
                if len(tc.Endpoints) > 0 { child.ReplaceEndpoints(tc.Endpoints) }
                cfg := tc.Discovery
                discovery.StartDNSDiscovery(ctx, discovery.DNSConfig{
                    Enabled:        true,
                    Service:        cfg.Service,
