...
func TestLogs_Aggregated_ExecuteQuery_ConcatsAndUnions(t *testing.T) {
    ...
    if len(res.Logs) != 2 { t.Fatalf("expected 2 rows, got %d", len(res.Logs)) }
    // union of fields should include service and host
    has := func(k string) bool { for _, f := range res.Fields { if f==k { return true } }; return false }
    if !has("service") || !has("host") { t.Fatalf("expected union fields include service and host: %v", res.Fields) }
}
...
func TestLogs_Aggregated_GetFields_Union(t *testing.T) {
    ...
    if err != nil { t.Fatalf("GetFields: %v", err) }
    if len(fields) != 3 { t.Fatalf("expected union size 3, got %d (%v)", len(fields), fields) }
}
...
func TestLogs_Aggregated_GetStreams_Union(t *testing.T) {
    ...
    if err != nil { t.Fatalf("GetStreams: %v", err) }
    if len(streams) < 2 { t.Fatalf("expected at least 2 stream labels, got %d (%v)", len(streams), streams) }
}
