...
func newFakeVM(t *testing.T, handlers map[string]http.HandlerFunc) *httptest.Server {
    ...
    } else { mux.HandleFunc("/api/v1/query", handlers["/api/v1/query"]) }
    ...
    } else { mux.HandleFunc("/api/v1/query_range", handlers["/api/v1/query_range"]) }

    // series
    if handlers["/api/v1/series"] == nil {
        mux.HandleFunc("/api/v1/series", func(w http.ResponseWriter, r *http.Request) {
            w.Header().Set("Content-Type", "application/json")
            _ = json.NewEncoder(w).Encode(struct{Status string `json:"status"`; Data []map[string]string `json:"data"`}{Status: "success", Data: []map[string]string{}})
        })
    } else { mux.HandleFunc("/api/v1/series", handlers["/api/v1/series"]) }

    // labels
    if handlers["/api/v1/labels"] == nil {
        mux.HandleFunc("/api/v1/labels", func(w http.ResponseWriter, r *http.Request) {
            w.Header().Set("Content-Type", "application/json")
            _ = json.NewEncoder(w).Encode(struct{Status string `json:"status"`; Data []string `json:"data"`}{Status: "success", Data: []string{}})
        })
    } else { mux.HandleFunc("/api/v1/labels", handlers["/api/v1/labels"]) }
...
func TestMetrics_Aggregated_ExecuteQuery_ConcatsResults(t *testing.T) {
    ...
    if err != nil { t.Fatalf("ExecuteQuery: %v", err) }
    if res == nil || res.Data == nil { t.Fatalf("no data") }
    // ensure both series are present
    m, _ := res.Data.(map[string]any)
    arr, _ := m["result"].([]any)
    if len(arr) != 2 { t.Fatalf("expected 2 series, got %d", len(arr)) }
}
...
func TestMetrics_Aggregated_ExecuteRange_SumsPoints(t *testing.T) {
    mk := func(points int) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
            ...
            for i := 0; i < points; i++ { vals = append(vals, []any{float64(i), float64(i)}) }
            series["values"] = vals
    ...
    if err != nil { t.Fatalf("ExecuteRangeQuery: %v", err) }
    if res.DataPointCount != 8 { t.Fatalf("expected 8 points, got %d", res.DataPointCount) }
}
...
func TestMetrics_Aggregated_Labels_Union(t *testing.T) {
    srvA := newFakeVM(t, map[string]http.HandlerFunc{
        "/api/v1/labels": func(w http.ResponseWriter, r *http.Request){
            w.Header().Set("Content-Type", "application/json")
            _ = json.NewEncoder(w).Encode(struct{Status string `json:"status"`; Data []string `json:"data"`}{Status:"success", Data: []string{"__name__","job"}})
        },
    })
    defer srvA.Close()
    srvB := newFakeVM(t, map[string]http.HandlerFunc{
        "/api/v1/labels": func(w http.ResponseWriter, r *http.Request){
            w.Header().Set("Content-Type", "application/json")
            _ = json.NewEncoder(w).Encode(struct{Status string `json:"status"`; Data []string `json:"data"`}{Status:"success", Data: []string{"instance","job"}})
        },
    })
    defer srvB.Close()
    ...
    if err != nil { t.Fatalf("GetLabels: %v", err) }
    if len(labels) != 3 { t.Fatalf("expected union size 3, got %d (%v)", len(labels), labels) }
}
...
func TestMetrics_Aggregated_LabelValues_Union(t *testing.T) {
    ...
    srvA := newFakeVM(t, map[string]http.HandlerFunc{"/api/v1/label/app/values": func(w http.ResponseWriter, r *http.Request){
        w.Header().Set("Content-Type", "application/json")
        _ = json.NewEncoder(w).Encode(struct{Status string `json:"status"`; Data []string `json:"data"`}{Status:"success", Data: []string{"api","worker"}})
    }})
    defer srvA.Close()
    srvB := newFakeVM(t, map[string]http.HandlerFunc{"/api/v1/label/app/values": func(w http.ResponseWriter, r *http.Request){
        w.Header().Set("Content-Type", "application/json")
        _ = json.NewEncoder(w).Encode(struct{Status string `json:"status"`; Data []string `json:"data"`}{Status:"success", Data: []string{"worker","cron"}})
    }})
    defer srvB.Close()
    ...
    if err != nil { t.Fatalf("GetLabelValues: %v", err) }
    if len(vals) != 3 { t.Fatalf("expected union size 3, got %d (%v)", len(vals), vals) }
}
