...
# Comprehensive E2E API Tests for Mirador Core v9.0.0 - Multi-Tenant RBAC & Identity Federation
# Compatible with Linux (Debian/RedHat) and macOS (Darwin)
# Updated for RBAC, Authentication, and Multi-Tenant Architecture
...
# Core API Tests with RBAC
test_core_apis_with_rbac() {
    log_phase "Core APIs with RBAC Enforcement"

    # Test 1: Health endpoints (should be public or admin-only)
    http_request "GET" "$BASE_URL/health" "200" "" "Health Check"
    http_request "GET" "$API_BASE/health" "200" "" "API Health Check"

    # Test 2: Metrics endpoints (protected)
    http_request "GET" "$API_BASE/metrics/names" "200" "" "Get Metric Names (Protected)"

    # Test 3: Logs endpoints (protected)
    http_request "GET" "$API_BASE/logs/fields" "200" "" "Get Log Fields (Protected)"

    # Test 4: Traces endpoints (protected)
    http_request "GET" "$API_BASE/traces/services" "200" "" "Get Trace Services (Protected)"
}
