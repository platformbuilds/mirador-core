...
- To test locally:
  - Build per-arch images and load: `make dockerx-build-local-multi VERSION=v10.0.1` → tags `...:v10.0.1-amd64` and `...:v10.0.1-arm64`.
  - Or publish a real multi-arch tag to a registry: `make dockerx-push VERSION=v10.0.1` and use that tag in compose/Helm.
  - Or export an OCI archive without pushing: `make dockerx-build VERSION=v10.0.1` → `build/mirador-core-v10.0.1.oci`.
