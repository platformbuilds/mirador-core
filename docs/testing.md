...
This guide covers the testing strategies and procedures for MIRADOR-CORE.
...
MIRADOR-CORE employs a comprehensive testing strategy covering functional tests, performance tests, integration tests, and database tests.

### Test Categories

- **Functional tests**: Default unit and handler tests (`go test ./...`)
- **Performance tests**: Benchmarks using `testing.B` (`go test -bench=. ./...`)
...
Automated performance regression tests ensure unified query engine components maintain acceptable performance baselines:
...
- **Correlation Queries**: P95 latency ≤ 50ms
...
- **Component Isolation**: Tests focus on individual components for reliable performance measurement
...
- All API tests ensure 100% endpoint coverage with comprehensive validation