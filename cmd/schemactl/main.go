...
func dump(ctx context.Context, c *client, outPath string) error {
    ...
    for _, cl := range classes {
        ...
        var resp struct{ Data struct{ Get map[string][]struct{ Additional struct{ ID string `json:"id"` } `json:"_additional"` } } }
        if err := c.do(ctx, http.MethodPost, "/v1/graphql", map[string]any{"query": q}, &resp); err != nil { return err }
        // pull the only array present
        var rows []struct{ Additional struct{ ID string `json:"id"` } `json:"_additional"` }
        ...
        for _, r := range rows {
            ...
            if err := c.do(ctx, http.MethodGet, "/v1/objects/"+r.Additional.ID, nil, &obj); err != nil { return err }
...
func restore(ctx context.Context, c *client, inPath string) error {
    ...
    for _, it := range items {
        ...
        if err := c.do(ctx, http.MethodPut, "/v1/objects/"+it.ID, payload, nil); err != nil { return fmt.Errorf("restore %s/%s: %w", it.Class, it.ID, err) }
    }
    return nil
}
